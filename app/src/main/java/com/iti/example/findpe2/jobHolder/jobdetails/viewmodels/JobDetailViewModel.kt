...
class JobDetailViewModel(private val request: ReceivedJobRequest?, private val job: Job?) :
    ...
    fun openChatRoom() {
        ...
        viewModelScope.launch {
            ...
            val companionEmail = FirebaseAuth.getInstance().currentUser?.email ?: FirebaseAuth.getInstance().currentUser?.providerData?.get(1)?.email!!
            val companionImageUrl = FirebaseAuth.getInstance().currentUser?.photoUrl!!
